...
This data package contains the data that powers the chart ["Human Development Index"](https://ourworldindata.org/grapher/human-development-index?tab=line&time=2010..2023&country=BRA~CHN~IND~MEX~ARG~URY~CHL~ESP~PRT~JPN~DEU~USA~KOR~ZAF~RUS&mapSelect=BRA~CHN~IND~MEX~ARG~URY~CHL~ESP~PRT~JPN~DEU~USA~KOR~ZAF~RUS&overlay=download-data&v=1&csvType=filtered&useColumnShortNames=false) on the Our World in Data website.
